...
class SwinTransformer3DFPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv6 = conv3x3(768, 256, stride=2, padding=1)  # P6
        self.conv7 = conv3x3(256, 256, stride=2, padding=1)  # P7

        self.ego_lateral = conv3x3(768,  256, stride=2, padding=0)
        self.avg_pool = nn.AdaptiveAvgPool3d((None, 1, 1))

        self.lateral_layer1 = conv1x1(768, 256)
    ...
    def init_weights(self, pretrained=None):
        ...
        #     logger.info(f'load model from: {self.pretrained}')
            ...
            if self.pretrained2d:
                print("---[SWIN-T]---  LOAD PRETRAINED 2D")
                # Inflate 2D model into 3D model.
                self.inflate_weights(logger)
            else:
                while(True):
                    print("---[SWIN-T]---  LOAD PRETRAINED 3D")
                # Directly load 3D model.
                # load_checkpoint(self, self.pretrained, strict=False, logger=logger)
                    self.load_state_dict(torch.load(self.pretrained))
        elif self.pretrained is None:
            print("---[SWIN-T]---  INIT FROM SCRTACH")
            self.apply(_init_weights)
        else:
    ...
    def forward(self, x):
        ...
        c6 = x
        # print('raw from swin-c3:', c3.shape)
        # print('raw from swin-c4:', c4.shape)
        # print('raw from swin-c5:', c5.shape)
        # print('raw from swin-c6:', c6.shape)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer3DFPN, self).train(mode)
        self._freeze_stages()


def swintfpn(args):
    model_type = args.MODEL_TYPE
    if model_type.startswith('swint'):
        if args.SWIN_PRETRAIN == '':
            args.SWIN_PRETRAIN = None
        logger.info("%%%%% SWIN-PRETRAIN: ", args.SWIN_PRETRAIN)
        return SwinTransformer3DFPN(
            pretrained=args.SWIN_PRETRAIN,
            pretrained2d=False)
    else:
